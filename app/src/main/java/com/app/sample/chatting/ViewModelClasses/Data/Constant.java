package com.app.sample.chatting.ViewModelClasses.Data;
...
import com.app.sample.chatting.ViewModelClasses.Model.Chat;
import com.app.sample.chatting.ViewModelClasses.Model.GroupDetails;
...
public class Constant {
    ...
    public static List<FriendsViewModel> getFriendsData(Context ctx)  {
        List<FriendsViewModel> items = new ArrayList<>();
        //String s_arr[]=
        ...
        for (int i = 0; i < s_arr.length; i++) {
            FriendsViewModel fr = new FriendsViewModel(i, s_arr[i], drw_arr.getResourceId(i, -1));
            items.add(fr);
        }
        return items;
    }
    ...
    public static List<Chat> getChatsData(Context ctx)  {
        ...
        for (int i = 0; i < 10; i++) {
            items.add(new Chat(i, s_date[i], true, new FriendsViewModel(s_arr[i+5], drw_arr.getResourceId(i+5, -1)), s_cht[i]));
        }
        return items;
    }
/*
...
*/
    private static ArrayList<FriendsViewModel> friendSubList(Context ctx, int start, int end){
        ArrayList<FriendsViewModel> friends = new ArrayList<>();
        friends.addAll(getFriendsData(ctx));
        ArrayList<FriendsViewModel> friends_ = new ArrayList<>();
...
/*
    public static List<ChatsDetails> getChatDetailsData(Context ctx, FriendsViewModel friend)  {
...
*/
    public static List<GroupDetails> getGroupDetailsData(Context ctx)  {
        ...
        List<FriendsViewModel> friends = getFriendsData(ctx);
